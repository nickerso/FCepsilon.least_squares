...
parameter_bounds = [len(initial_params)*[0], len(initial_params)*[6.0]]
...
optimal_params=least_squares(model_function_lsq, initial_params, args=(expt_time,expt_pSyk,expt_pGRB2, 'optimisation'),
                               bounds=parameter_bounds,xtol=1e-4,verbose=1)

f =model_function_lsq(initial_params, expt_time, expt_pSyk, expt_pGRB2,'visualisation', debug=True)
print(f)
...
#solution = model_function_lsq(expt_time, *optimal_params)
# Plot the results
#y_true =model_function_lsq (expt_time, a, b, c)

#plt.plot(expt_time, expt_pGRB2, '.', label='Experiment pGRB2')
#plt.plot(expt_time, f[1], '*', label='Model pGRB2')

#plt.plot(expt_time, f, '--', label='Optimal')


    
#optimal_params, covariance = curve_fit(model_function, expt_time, expt_pGRB2, p0=initial_params, bounds=parameter_bounds)
#print('Optimal parameters:')
#for n, v in enumerate(optimal_params):
#   print('  {}: {:g} ({:g})'.format(parameter_names[n], v, initial_params[n]))
# Plot the results
#solution = model_function(expt_time, *optimal_params)
#unfitted = model_function(expt_time, *initial_params)
#plt.plot(expt_time, expt_pGRB2, '.', label='Experiment')
#plt.plot(expt_time, unfitted, '-', label='Model')
#plt.plot(expt_time, solution, '--', label='Optimal')
#plt.plot(expt_time, expt_pGRB2-solution, '-', label='Error')
#plt.legend(loc='best')
#plt.show()
